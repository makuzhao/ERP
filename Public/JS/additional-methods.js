...
/* 正数浮点型、RMB  */
jQuery.validator.addMethod("numFormat", function (value, element) {
    return this.optional(element) || /^[0-9]{1,10}(\.\d{1,2})$/.test(value) || /^[0-9]+$/.test(value);
}, $.validator.format("请输入合法的数字,小数精度小数点后二位"));

/*  手机 、座机 */
jQuery.validator.addMethod("phoneZJi", function (value, element) {
    return this.optional(element) || /^(13|15|18)\d{9}$/i.test(value) || /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/i.test(value);
}, $.validator.format("格式不正确,请使用下面格式:020-88888888或手机号码."));

/* 11位手机号码 */
jQuery.validator.addMethod("shouJi", function (value, element) {
    return this.optional(element) || /^(13|15|18)\d{9}$/i.test(value);
}, $.validator.format("手机号码不正确"));

/* 验证非零的正整数：^[1-9][0-9]*$   */
jQuery.validator.addMethod("feiLingZ", function (value, element) {
    return this.optional(element) || /^[1-9][0-9]*$/.test(value);
}, $.validator.format("非零的正整数"));
